=== WooCommerce PDF & Print ===
Contributors: gVectors Team
Donate link: https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=VNMGTCF9NRH5C
Tags: WooCommerce, WooCommerce Product, WooCommerce Product PDF, PDF, Save as PDF, Save Product as PDF, PDF Button, WooCommerce Product Word Document, Word Document, Doc, Save as Doc, Save as Word Document, Save Product as Doc, Doc Button, Word Document Button, Print, Product Print, WooCommerce Print, Print Button
Requires at least: 2.7.0
Tested up to: 4.2
Stable tag: 1.4.3

Adds PDF, Doc and Print buttons on WooCommerce Product Page. Allows print and save product information as PDF and MS Doc file.

== Description ==
Adds PDF, Doc and Print buttons on WooCommerce Product Page. Allows print and save product information as PDF and MS Doc file.

**Main Functions and Features**

* Saves product details as PDF file,
* Saves product details as Word Document,
* Includes all type of product attributes (Simple, Taxonomy, Variable),
* Includes content of custom tabs ( Woocommerce Product Tabs plugin ),
* Flexible options for appearance saving and localization of buttons and strings, 
* PDF export Extension for Documents in Russian Language
* Prints Product Details,
* Prints Template settings 

For Saving Wordpress Posts and Pages as PDF, MS Doc, Text, HTML, XMS and Printing please UPM edition:
Universal Post Manager https://wordpress.org/plugins/universal-post-manager/

== Installation ==

  * Extract "woocommerce-pdf-print.zip" archive.
  * Upload the woocommerce-pdf-print folder to the "/wp-content/plugins/" directory .
  * In your WordPress administration, go to the Plugins page.
  * Activate the WooCommerce PDF & Print plugin through the 'Plugins' menu in WordPress and a menu WooPP  whith three submenus will appear in your admin panel menus.

== Screenshots ==

1.  Print and Save Middle Size Buttons on Product Page Screenshot #1
2.  Print and Save Icons Buttons on Product Page Screenshot #2
3.  Print and Save Simple Buttons on Product Page Screenshot #3
4.  PDF Settings Page Screenshot #4
5.  General Settings Page Screenshot #5

== FAQ ==

= Please Check the Following WooPP Resources =

* Support Forum: <http://gvectors.com/questions/>

== Changelog ==

= 1.4.3 =

* Fixed Bug : Contact for buttons in print document
* Fixed Bug : Show product attributes with their names instead of their slug

= 1.4.2 =

* Fixed Bug : Hide invisible product attributes in save/print documents
* Fixed Bug : Strip shortcodes and form in product description

= 1.4.1 =

* Fixed Bug : Translation issues with document statis phrases
* Fixed Bug : Problem with General Custom Tabs created by Woocommerce Product Tabs plugin

= 1.4.0 =

* Added : Ability to add document header image (logo)
* Added : Options to show/hide each component of saved/printed document
* Fixed Bug : Problem with document static phrases
* Fixed Bug : Issues with Russian language
* Fixed Bug : Different language character issues on PDF document

= 1.3.3 =

* Fixed Bug : Can't open file /unifont/DejaVuSansCondensed.ttf

= 1.3.2 =

* Fixed Bug : Error - can't find fond file

= 1.3.1 =

* Fixed Bug : Again - not saved document static phrases on 1.2.3 update process
* Added : Save as PDF Supports all languages (tested 64) Česky, Slovenščina, Հայերեն, მარგალური, עברית, Македонски, العربية, 中文, Ελληνικά, Türkçe, অসমীযা়  ...

= 1.2.3 =

* Fixed Bug : Black background problem with transparent PNG images in PDF document
* Fixed Bug : Another 14 currency symbols displaying problem in PDF document
* Fixed Bug : Not saved document static phrases on 1.2.0 update process

= 1.2.0 =

* Added : Ability to edit all static phrases in save and print documents
* Added : Russian language autodetection and support
* Fixed Bug : Currency symbol displaying problem on PDF document
* Fixed Bug : HTML character displaying problem on PDF document
* Compatibility with Wordpress 4.1 version

= 1.1.1 =

* Added : Supports taxonomy based product attributes
* Added : Supports Custom Tabs created by Woocommerce Product Tabs plugin
* Added : Supports Wordpress the_content and do_shortcode filter/method on exported product description
* Fixed Bug : FPDF error - Some data has already been output, can't send PDF file

= 1.0.2 =

* Fixed Bug : Page Not Found error on Doc
* Fixed Bug : PDF export problem with Alpha channel PNG images
* Fixed Bug : PDF export error - Missing or incorrect image file
* Fixed Bug : Print document CSS issues

= 1.0.1 =

* Fixed Bug : Page Not found error on Doc Export
* Fixed Bug : PDF export problem when PHP 'allow_url_fopen' is off

= 1.0.0 =

Initial version.
