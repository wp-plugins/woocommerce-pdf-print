...
function wpp_export( $export ) {
	...
	elseif( $export == 'pdf' ) {
		...
		//Price Style
		if( $product->is_on_sale() ){ $wpp_price = str_replace(array('<del>', '</del>'), array('^[font]['.get_option( 'wpp_ph_real_price' ).': ', '][200,200,200][14]^<br>
 ^[font]['.get_option( 'wpp_ph_curr_sale_price' ).':'), str_replace( '&ndash;', ' - ', $product->get_price_html())).'][15, 117, 84][14]^';
		} else{ $wpp_price = '^[font]['.get_option( 'wpp_ph_price' ).': ' .  strip_tags( $product->get_price_html()).'][15, 117, 84][14]^'; }
